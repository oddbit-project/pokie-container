import pytest
from pokie_container.constants import SVC_CONTAINER, TREE_DEFAULT, TENANT_DEFAULT, NODE_DEFAULT
...
class TestPlatform:
    ...
    def test_node(self, svc_container: ContainerService):
        ...
        record = svc_container.add_node_type(TREE_DEFAULT, id_node_type, node_type_label)
        assert record is not None

        # create 10 nodes at root level, 5 nodes at second level
        nodes = {}
        for i in range(0, 10):
            record = svc_container.add_node(0, id_node_type, "root node {}".format(str(i)))
            nodes[record.id] = []
            for j in range(0, 2):
                child = svc_container.add_node(0, id_node_type, "child node {} for node {}".format(str(j), str(i)),
    ...
    def test_node_tree_multilevel(self, svc_container: ContainerService):
        ...
        #    \ /
        #     F
        node_f = svc_container.add_node(0, id_node_type, "node F", id_parents=[node_d.id, node_e.id])
    ...
    def test_node_tree_multi_tree(self, svc_container: ContainerService):
        ...
        record = svc_container.add_node_type(tree_1, tree1_id_node_type, tree1_node_type_label)
        assert record is not None

        tree2_id_node_type = 11
        tree2_node_type_label = "custom type for tree2"
        record = svc_container.add_node_type(tree_2, tree2_id_node_type, tree2_node_type_label)
        assert record is not None
        ...
        node_b1 = svc_container.add_node(0, tree1_id_node_type, "node B1", id_parents=node_a1.id)
        node_c1 = svc_container.add_node(0, tree1_id_node_type, "node C1", id_parents=node_a1.id)
        ...
        node_b2 = svc_container.add_node(0, tree2_id_node_type, "node B2", id_parents=node_a2.id)
        node_c2 = svc_container.add_node(0, tree2_id_node_type, "node C2", id_parents=node_a2.id)
    ...
    def test_node_parent(self, svc_container: ContainerService):
        ...
        tree2_node_type_label = "custom type for tree2"
        record = svc_container.add_node_type(id_tree_2, tree2_id_node_type, tree2_node_type_label)
        assert record is not None
        ...
        node_d2 = svc_container.add_node(0, tree2_id_node_type, "node D2", id_parents=node_b2.id)
        node = svc_container.get_node(node_d2.id)
        assert isinstance(node, dict)
        assert isinstance(node['record'], NodeRecord)
    ...
    def test_node_src(self, svc_container: ContainerService):
        ...
        node_b = svc_container.add_node(0, NODE_DEFAULT, "node B1", id_parents=node_a.id, src=101)
        node_c = svc_container.add_node(0, NODE_DEFAULT, "node C1", id_parents=node_a.id, src=102)
        node_d = svc_container.add_node(0, NODE_DEFAULT, "node D1", id_parents=node_b.id, src=103)
